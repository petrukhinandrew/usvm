...
class UTestExecutorCollectingState(jcClasspath: JcClasspath, ucp: URLClassPathLoader) : UTestExecutor(jcClasspath, ucp) {
    ...
    override fun buildExecutionState(
        ...
        val instanceDescriptor = callMethodExpr.instance?.let { buildDescriptorFromUTestExpr(it, executor).getOrNull() }
        val argsDescriptors = callMethodExpr.args.map { buildDescriptorFromUTestExpr(it, executor).getOrNull() }
